...
const SelectedOperators: FC<SelectedOperatorsProps> = ({ clusterSize }) => {
  ...
  const totalYearlyFee = selectedOperators.reduce((sum, operator) => sum + parseFloat(operator.fee), 0).toFixed(2);
  return (
    <Card className="flex-shrink-0 w-full lg:w-1/4">
      <CardContent className="p-6">
        ...
        <div className="space-y-4">
          {selectedOperators.map((operator) => (
            <div
              ...
              className={`flex justify-between items-center
                         ${
                           operator.dkg_address === '' || operator.dkg_address.startsWith('http://')
                             ? 'bg-red-400/50 '
                             : ''
                         } bg-muted/50
                          rounded-lg p-3`}
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className={`text-xs ${operator.dkg_address === '' ? '' : ''} text-muted-foreground`}>
                    Fee: {(parseFloat(operator.fee) / 1e9).toFixed(2)} SSV
                  </p>
                  {operator.dkg_address === '' && <p className="text-sm font-bold">does not support dkg</p>}
                </div>
              </div>
              <button
          ...
          <div className="text-sm font-semibold">Total Yearly Fee: {totalYearlyFee} SSV</div>
          {selectedOperators.some((op) => !(op.type === 'verified_operator') || operatorUsesHTTP(op)) && (
            <div className="mt-4 bg-yellow-100 border border-yellow-300 rounded p-3 text-sm text-gray-900 ">
          ...
          {selectedOperators.some((op) => operatorUsesHTTP(op)) && (
            <div className="mt-4 bg-red-100 border border-red-300 rounded p-3 text-sm dark:text-gray-900 ">
              You have selected one or more operators that use HTTP. This is not secure.
            </div>
          )}
        </div>
        <div className="flex w-full justify-center items-center p-2">
          <Button
            disabled={
              selectedOperators.length !== clusterSize ||
              selectedOperators.some((op) => op.dkg_address === '' || operatorUsesHTTP(op))
            }
            onClick={() => {
              navigate('/join/generate', {
                replace: true,
